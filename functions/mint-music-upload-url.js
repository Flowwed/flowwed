export async function handler(event) {
  const SUPABASE_URL = "https://octwwpatppbenqwkcqaw.supabase.co";
  const SUPABASE_KEY =
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9jdHd3cGF0cHBiZW5xd2tjcWF3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTg5NjYxMjYsImV4cCI6MjA3NDU0MjEyNn0.kYX1yCkx3Zl2J_qLHZYcknLnx_aXl26zB--__MzkknI";

  try {
    const body = JSON.parse(event.body || "{}");

    // ⬇️ ВАЖНО: page_key вместо slot
    const { slug, token, page_key, filename } = body;

    if (!slug || !token || !page_key || !filename) {
      return {
        statusCode: 400,
        body: JSON.stringify({
          error: "slug, token, page_key, filename required",
        }),
      };
    }

    // ⬇️ ПРАВИЛЬНЫЙ PATH ДЛЯ МУЗЫКИ
    const musicPath = `${slug}/${page_key}/${filename}`;

    // ⬇️ ВЫЗЫВАЕМ SUPABASE EDGE FUNCTION (ТА ЖЕ, ЧТО ДЛЯ ФОТО)
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/mint-upload-url`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${SUPABASE_KEY}`,
        },
        body: JSON.stringify({
          bucket: "music",          // ✅ ДРУГОЙ BUCKET
          path: musicPath,          // ✅ ДРУГОЙ PATH
          token,
        }),
      }
    );

    const data = await response.json();

    if (!response.ok) {
      return {
        statusCode: response.status,
        body: JSON.stringify({
          error: data.error || "Upload service rejected the request.",
          details: data,
        }),
      };
    }

    // ⬇️ ОТДАЁМ uploadUrl + path НА ФРОНТ
    return {
      statusCode: 200,
      body: JSON.stringify({
        uploadUrl: data.signedUrl || data.url,
        path: musicPath,
      }),
    };
  } catch (err) {
    return {
      statusCode: 500,
      body: JSON.stringify({
        error: "Failed to contact Supabase function.",
        details: err.message,
      }),
    };
  }
}
